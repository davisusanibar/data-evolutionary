...
- **SC-002** — build verde en ~1 s; `OrderEvent.java` y `CustomerRevenueWindow.java`
  generados desde los `.avsc`.
...
- **SC-004** — 5 s en ambas variantes, frente al umbral declarado de 30 s.
- **SC-005** — 42/42 checks, exit 0.
- **SC-006** — el job en el cluster real produce exactamente la referencia
  (tabla más abajo).
...
| SC-005 | pin de la constitución alterado un carácter; y FR-005 sin cobertura | 41/42 y 40/42 respectivamente, verde al restaurar |

## El resultado end-to-end

El job se sometió al cluster Flink 1.20.2 del repositorio
(JobID `2947066e4ab33b57df57b9682b88ee68`, estado RUNNING), leyendo Avro desde
Kafka a través del Schema Registry y escribiendo Avro de vuelta:
...
`evidence/cierre-owner.txt`. El cierre no autoriza commit ni push, que siguen
