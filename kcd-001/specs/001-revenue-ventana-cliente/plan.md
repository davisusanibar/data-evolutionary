...
| II. Spec antes que artefacto | PASS | La spec declara FR-001..FR-006, SC-001..SC-006 y owner antes de que exista una sola clase del job en `kcd-001`. |
...
| IV. Cero cifras sin fuente | PASS | La única cifra de negocio (revenue por ventana) se compara en SC-001 contra un cálculo de referencia sobre un fixture determinista versionado; las demás afirmaciones tienen fuente en `evidence/sources.md`. |
...
| VI. La IA consume; el humano firma | PASS | La feature avanza por fases con estado Propuesto; el paso a implementado y el cierre exigen la firma del owner declarado. |
...
Constitución verificada: 1.0.0 · sha256:96c6e0bff817c9257dc6ac24cd7a71eccc155dfa40e6856f8ec5762e7354680f

## Arquitectura de la solución

El job vive en el módulo `kcd-001` y reutiliza la infraestructura de
`infra/dockercompose` sin añadir servicios (FR-006).
...
1. **Fuente** — tópico Kafka de órdenes, deserializado con
   ...
   `http://registry:8081` (hostname `registry` según
   `infra/dockercompose/docker-compose.yml`), siguiendo el patrón ya probado en
...
2. **Watermarks** — estrategia de marca de agua con desorden acotado, derivada
   ...
   forma explícita en el código y se documenta junto al job (FR-002).
...
Contratos, en `src/main/resources/model/` del módulo `kcd-001`:
...
El `avro-maven-plugin` (1.11.3, la misma versión ya validada en
`data-cdc-kafka-flink-iceberg`) se engancha en `generate-sources` y escribe en
`target/generated-sources/avro` (FR-004). El `pom.xml` de `kcd-001` ya declara
Flink 1.20.1 y el connector Kafka; este plan añade las dependencias Avro y de
Schema Registry replicando la configuración del módulo CDC.
...
sobre una copia de trabajo del `.avsc`, y siempre revierte (FR-005):
...
4. Mostrar el mensaje de error señalando el símbolo desaparecido (SC-003).
...
- **Puerta de compilación (contrato Avro)** — pasa a ser significativa en
  `kcd-001`, que hasta ahora no declaraba Avro. Añadir `avro-maven-plugin` y las
...
- **Gate SDD `consumer-release`** — esta feature es la primera bajo `specs/` del
  consumer `kcd-001`. A partir de aquí, el gate deja de reportar "sin features
  versionadas" y pasa a validar contrato, cobertura y cierre. Verificado por
  SC-005.
...
- **Sin impacto** sobre los casos de `data-cdc-kafka-flink-iceberg` de KCD 2025:
...
5. Verificación end-to-end sobre la infraestructura real (SC-006).
6. Cierre gobernado: gate SDD verde (SC-005) y firma del owner.
