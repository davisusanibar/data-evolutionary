...
fuentes:
  - pom.xml
  - specs/001-revenue-ventana-cliente/evidence/sources.md
...
Este repositorio se llevó a la KCD 2025 sin SDD. Allí se demostró un pipeline
**CDC + Join** operativo en el módulo `data-cdc-kafka-flink-iceberg`
(`e_cdckafkaflink.JobStreamingCDCKafkaFlink`): órdenes desde Kafka en Avro,
pagos desde Postgres vía CDC, join en ventana y sinks Avro de vuelta a Kafka.
Toda la infraestructura (Kafka, Schema Registry, Postgres, Flink, MinIO, Hive)
se levanta con `infra/dockercompose/docker-compose.yml`. El pipeline funcionaba.
...
Esta feature añade al módulo `kcd-001` un caso de uso nuevo — **revenue por
cliente en ventana temporal** — construido bajo gobierno EDAIOS SDD, y lo usa
como soporte para demostrar lo contrario: que el contrato Avro es una
**barrera de compilación**, y que la deriva se detecta antes de que exista un
job en ejecución.
...
- Un job Flink en `kcd-001` que agrega revenue por cliente sobre ventanas de
  event-time.
- Contratos Avro versionados en el módulo, que generan código en el build.
- Un procedimiento de canary reproducible que introduce deriva y demuestra el
  fallo cerrado del build.

Queda fuera de alcance:

- Reescribir o modificar los casos de uso de `data-cdc-kafka-flink-iceberg`
  llevados a KCD 2025.
- Persistencia en Iceberg del resultado agregado (posible feature posterior).
- Levantar servicios de infraestructura nuevos fuera de
  `infra/dockercompose`.
...
2025 usó ventanas de processing-time, suficiente para una demo de join. Aquí
