...
public class IcebergInsertDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Record> records = createSampleRecords(table);

        OutputFile parquetFile = writeBatchToParquet(table, records);

        DataFile newDataFile = commitParquetToTable(table, parquetFile, records.size());
    ...
    private static List<Record> createSampleRecords(Table table) {
        ...
        record1.setField("order_id", 1L);
        record1.setField("status", PARTITION_STATUS_NEW);
        record1.setField("amount", BigDecimal.valueOf(100.00).setScale(2));

        Record record2 = GenericRecord.create(table.schema());
        record2.setField("order_id", 2L);
        record2.setField("status", PARTITION_STATUS_NEW);
        record2.setField("amount", BigDecimal.valueOf(200.00).setScale(2));
